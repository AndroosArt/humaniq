(function (global) {
  ...
  function buildFollowUpMessage(evaluation) {
    const firstName = normalizeString(evaluation.contactName || 'there');
    ...
    if (evaluation.tier === 'medium') {
      return {
        ...
        body: [
          ...
          '',
          'Thanks for submitting your Growth & Conversion Review.',
          `We can already see a few likely friction points around ${evaluation.primaryProblem.toLowerCase()}, and we are preparing a tailored recommendation for the best next step.`,
          '',
